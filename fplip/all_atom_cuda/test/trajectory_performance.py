...
class TrajectoryAnalyzerPerformanceTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.tpr = str(TEST_DATA_DIR / "pull/pull.tpr")
        cls.xtc = str(TEST_DATA_DIR / "pull/pull_center.xtc")
        cls.gro = str(TEST_DATA_DIR / "pull/pull.gro")
    ...
    def test_pdb_conversion_performance(self):
        ...
        for _ in range(3):
            start = time.perf_counter()
            converter = PDBConverter(filter_atoms_by_chains(self.analyzer.u.atoms, ['A', 'B', 'CL']))
            pdb_str = converter.fast_convert()
            elapsed = time.perf_counter() - start
            times.append(elapsed)

        print(f"\n[PERF] PDB conversion (frame 0, filtered chains A/B/CL):")
        print(f"  Average: {np.mean(times)*1000:.2f} ms")
        print(f"  Min: {np.min(times)*1000:.2f} ms")
        print(f"  Max: {np.max(times)*1000:.2f} ms")
    ...
    def test_kdtree_alignment_performance(self):
        ...
        print(f"\n[PERF] KD-tree alignment (filtered chains A/B/CL):")
    ...
    def test_coordinate_update_performance(self):
        ...
        self.analyzer.setup_detector(backend=NumPyBackend())
        ...
        print(f"\n[PERF] Coordinate update per frame (with waters):")
    ...
    def test_detect_all_performance(self):
        ...
        print(f"\n[PERF] detect_all per frame (with waters):")
        print(f"  Average: {np.mean(times)*1000:.2f} ms")
        print(f"  Min: {np.min(times)*1000:.2f} ms")
        print(f"  Max: {np.max(times)*1000:.2f} ms")
